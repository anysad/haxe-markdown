...
class InlineParser {
	static var defaultSyntaxes = [
		...
		// Encode "&".
		new TextSyntax('&', '&amp;'),
		// Leave HTML and HTML comments as is.
		new TextSyntax('<(?:!--[ ]*|/)?\\w+.*?>'),
...
class TagSyntax extends InlineSyntax {
	...
	override function onMatch(parser:InlineParser):Bool {
		parser.stack.push(new TagState(parser.pos, parser.pos + pattern.matched(0).length, this));
		return true;
	}
...
class TagState {
	...
	// pattern.
	public function tryMatch(parser:InlineParser):Bool {
		if (syntax.endPattern.match(parser.currentSource) && (syntax.endPattern.matchedPos().pos == 0)) {
			// Close the tag.
			close(parser);
			return true;
		}
